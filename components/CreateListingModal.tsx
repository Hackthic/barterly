...
const CreateListingModal: React.FC<CreateListingModalProps> = ({ isOpen, onClose, onListingCreated }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-8 max-w-lg w-full m-4 relative animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="file" id="image" accept="image/*" onChange={e => { setImageFile(e.target.files ? e.target.files[0] : null); setError(''); }} className="mt-1 block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 dark:file:bg-indigo-900/50 file:text-indigo-700 dark:file:text-indigo-300 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-900/80" required/>
          </div>
