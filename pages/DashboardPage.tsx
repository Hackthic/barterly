                            ...
                            <Link to={`/chat/${chat.id}`} className="block p-4 bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center">
                                       <img src={chat.product.image} alt={chat.product.title} className="w-12 h-12 rounded-md object-cover mr-4" />
                                        <div>
                                            <p className="font-semibold">{chat.product.title}</p>
                                            <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{chat.lastMessage}</p>
                                        </div>
                                    </div>
                                    <span className="text-xs text-slate-400 dark:text-slate-500">{new Date(chat.lastMessageTimestamp).toLocaleTimeString()}</span>
                                </div>
                            </Link>
                        </li>
                    ))}
